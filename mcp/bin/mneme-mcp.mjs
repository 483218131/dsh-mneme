#!/usr/bin/env node
/**
 * mneme-mcp —— mneme-memory 独立 stdio MCP server（v1，#181；#300 双包拆出）
 *
 * 零依赖 ESM（Node >= 20，全局 fetch）。任何 MCP 客户端（Claude Code / Cursor /
 * Codex / Hermes / OpenCode / OpenClaw 等任意支持 Model Context Protocol 的
 * host）以 stdio 挂载本进程，即可获得与 DSH 内一致的记忆工具六件套：
 * memory_save / memory_search / memory_list / memory_get / memory_update /
 * memory_delete。
 *
 * 数据面走 dsh-mneme 插件的 standalone API（8790，Bearer）而非直连 memory.db
 * ——写入并发由 DSH 单点负责，最稳。工具名、参数、输出渲染与插件包
 * dsh-mneme/src/tools.js 逐字对齐（插件包 test/mcp-package.test.js 有跨包
 * 平价回归锁漂移）；scope 语义差异仅一处：standalone API 无会话上下文，
 * save 不做自动标注、只认显式 agent_scope / workspace_scope 声明。
 *
 * 配置优先级（沿用 CLI 约定）：
 *   环境变量 DSH_MNEME_URL / DSH_MNEME_TOKEN（兼容老挂载）
 *   > MNEME_URL / MNEME_TOKEN（新包名别名）
 *   > ~/.dsh-mneme/cli.json（{"url","token"}）
 *   > 默认 http://127.0.0.1:8790
 *
 * Claude Code 挂载示例（.mcp.json）：
 *   { "mcpServers": { "mneme-memory": {
 *       "command": "mneme-mcp",
 *       "env": { "MNEME_TOKEN": "<面板「设置 → 外部访问 API」中的 token>" } } } }
 *
...
const UNAUTHORIZED_HINT =
  ...
  "or MNEME_TOKEN (or `dsh-mneme config set <url> <token>`); the token is shown in " +
  "the DSH panel under Settings -> External API.";
...
export function resolveMcpConfig(env = process.env, configPath = CONFIG_PATH) {
  ...
  // env 别名（#300 双包）：新包名 mneme-memory 后用 MNEME_URL/MNEME_TOKEN；
  // DSH_MNEME_* 保持优先（老用户挂载配置零迁移）。config 文件不收新键。
  const url = firstNonEmpty(env.DSH_MNEME_URL, env.MNEME_URL, file.url) ?? DEFAULT_URL;
  const token = firstNonEmpty(env.DSH_MNEME_TOKEN, env.MNEME_TOKEN, file.token) ?? "";
...
function createMcpServer(config) {
  ...
  return {
    ...
    start() {
      ...
      if (!config.token) {
        process.stderr.write("[mneme-mcp] warning: no token configured (DSH_MNEME_TOKEN / MNEME_TOKEN / ~/.dsh-mneme/cli.json); authenticated calls will fail.\n");
      }
    }
  };
}
